...
class pulsar_video_widgets extends WP_Widget {
	function pulsar_video_widgets() {
		...
		$widget_ops = array('classname' => 'pulsar-videos','description' => esc_attr__('Video Widget - supports Youtube, Vimeo, Dailymotion','quantumtheme'));
		/* $control_ops = array( 'twitter name' => 'pulsar', 'count' => 3, 'avatar_size' => '32' ); */		
		parent::__construct('pulsar-videos',esc_attr__('[Micro Themes] - Videos','quantumtheme'),$widget_ops);
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_attr__( 'Video', 'quantumtheme' ) : $instance['title'], $instance, $this->id_base );
...
function form( $instance ) {
		...
		$defaults = array( 'title' => esc_attr__('Videos','quantumtheme'), 
			'title' => 'Popular Video', 
        ...
        <p>
        <label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php esc_attr_e('Video Type:', 'quantumtheme') ?></label>
        <select id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" class="widefat">
            <option <?php if ( 'Youtube' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Youtube', 'quantumtheme') ?></option>
            <option <?php if ( 'Vimeo' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Vimeo', 'quantumtheme') ?></option>
            <option <?php if ( 'Dailymotion' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_attr_e('Dailymotion', 'quantumtheme') ?></option>
        </select>
        </p>

		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'id' )); ?>"><?php esc_attr_e('Video ID:', 'quantumtheme') ?></label>
		<input id="<?php echo esc_attr($this->get_field_id( 'id' )); ?>" name="<?php echo $this->get_field_name( 'id' ); ?>" value="<?php echo esc_attr($instance['id']); ?>" class="widefat" />
		</p>
